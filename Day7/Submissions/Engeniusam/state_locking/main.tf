...
resource "aws_instance" "web_server" {
  ...
  # Leave the first part of the block unchanged and create our `local-exec` provisioner
  provisioner "local-exec" {
    command = "powershell.exe -Command \"Set-ItemProperty -Path .\\${local_file.private_key_pem.filename} -Name IsReadOnly -Value `$false; icacls .\\${local_file.private_key_pem.filename} /inheritance:r /grant:r `$($env:USERNAME):(R)\""
  }
