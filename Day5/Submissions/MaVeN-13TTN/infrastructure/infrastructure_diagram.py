...
def create_infrastructure_diagram():
    ...
    with Diagram(
        ...
        with Cluster("AWS Region: us-east-1"):
            ...
            with Cluster("VPC (Default)"):
                ...
                with Cluster("Multi-AZ Deployment (5 AZs)"):
                    ...
                    # Auto Scaling Group
                    with Cluster(f"Auto Scaling Group\nMin: {config['min_size']}, Max: {config['max_size']}, Desired: {config['desired_capacity']}"):
                        
                        # EC2 Instances
                        instances = []
                        for i, az in enumerate(config["availability_zones"][:3]):  # Show 3 AZs for clarity
                            with Cluster(f"Availability Zone: {az}"):
                                if i < config["desired_capacity"]:
                                    instance = EC2(f"Web Server {i+1}\n{config['instance_type']}")
                                    instances.append(instance)
                                else:
                                    instance = EC2(f"Auto Scale Instance\n{config['instance_type']}")
                                    instances.append(instance)
                
                # Security Group for instances
                instances_sg = Firewall("Instances Security Group\nHTTP:80 from ALB only")
                ...
                with Cluster("Monitoring & Alarms"):
                    ...
                    unhealthy_alarm = Cloudwatch("Unhealthy Hosts Alarm\n>0 unhealthy targets")
        ...
        instances_sg >> Edge(label="Protects", style="dashed", color="orange") >> instances[0]
        ...
        low_cpu_alarm >> Edge(label="Scale Down (-1)", color="blue", style="bold") >> asg
